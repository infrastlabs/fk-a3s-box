    ...
    fn convert_poll_message(
        ...
        if let Some(authority) = pseudo.authority {
            match uri::Authority::from_maybe_shared(authority.clone().into_inner()) {
                Ok(value) => parts.authority = Some(value),
                Err(why) => {
                    // gRPC clients (grpc-go >= 1.57, used by crictl/kubelet)
                    // connecting over a Unix domain socket send the
                    // percent-encoded socket path as the :authority
                    // pseudo-header (e.g. "%2Frun%2Ffoo.sock"), which is not a
                    // valid HTTP authority. Tolerate UDS-style authorities by
                    // dropping the value instead of resetting the stream with
                    // PROTOCOL_ERROR; gRPC routes by :path, so authority is
                    // unused server-side. See hyperium/h2#487, grpc/grpc#38142.
                    let raw = authority.as_str();
                    let looks_like_uds = raw.is_empty()
                        || raw.starts_with('/')
                        || raw.contains("%2F")
                        || raw.contains("%2f")
                        || raw.ends_with(".sock");
